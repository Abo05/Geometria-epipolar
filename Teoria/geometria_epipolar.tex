...
Las imágenes utilizadas están realizadas por cámaras con la misma referecia cartesiana
y distancia focal.

En este programa hacemos el cálculo de la matriz fundamental con el 
algorimto de los 8 puntos normalizado y creamos una estimación robusta con RANSAC.


\subsubsection{Rectificación de imágenes}
...
\begin{enumerate}
    \item \textbf{Traslación:} Se traslada el centro de la imagen al
    origen de coordenadas.
    \item \textbf{Rotación:} Se rota la imagen para que el epípolo,
    expresado en coordenadas relativas al centro, quede alineado con
    el eje $X$ positivo.
    \item \textbf{Transformación proyectiva:} Se aplica una
    transformación $G$ que envía el epípolo al infinito.
\end{enumerate}
...
\begin{definition}[Disparidad]
    Dados dos puntos correspondientes $\mathbf{x}_L = (x, y)$ en la
    imagen izquierda y $\mathbf{x}_R = (x', y)$ en la imagen derecha,
    se define la disparidad como:
    \[
        d = x - x'
    \]
\end{definition}
...
Suponemos, sin perdida de generalidad que el centro óptico de la cámara izquierda
...
\begin{itemize}
    \item \textbf{Oclusiones:} regiones visibles desde una cámara
    pero ocultas desde la otra por un objeto interpuesto. Son
    inevitables en cualquier par estéreo con baseline no nula.
    \item \textbf{Regiones sin textura:} zonas uniformes donde la
    métrica de comparación produce el mismo coste para múltiples
    disparidades. En estos casos se puede aplicar un test de unicidad 
    que descarta el match si el segundo mejor candidato es demasiado parecido
    al mejor.
\end{itemize}
